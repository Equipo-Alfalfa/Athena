...
def categorizar():
    with open('config.json', 'r') as f:
        config = json.load(f)
    subtemas_palabras_clave = config['subtemas_palabras_clave']
    umbrales_personalizados = config['umbrales_personalizados']
        # Vectorizar corpus
    vectorizer = TfidfVectorizer()
    corpus = df["text"].tolist()  # Lista todos los textos
    vectorizer.fit(corpus)  # 

        # categorizacion a cada articulo
    df["label"] = df["text"].apply(
        ...
        # guardar df con nuevas etiquetas
